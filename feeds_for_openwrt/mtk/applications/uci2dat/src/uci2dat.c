...
#define PARSE_UCI_LIST(dst) \
    do { \
        struct uci_option *o = NULL; \
        o = uci_lookup_option(uci_ctx, s, dst.uci_key); \
        if(o) { \
			if (o->type == UCI_TYPE_LIST) { \
				struct uci_element *e; \
				int a = sizeof(dst.value); \
				char *p = dst.value; \
				int sep = 0; \
				int len; \
				uci_foreach_element(&o->v.list, e) { \
            		len = snprintf(p, a, "%s%s", (sep == 0) ? "" : ";", e->name); \
					a -= len; \
					p += len; \
					sep = 1; \
				} \
...
typedef struct _vif
{
	...
	param macfilter;	   /* mac filter policy */
	param maclist;	   	   /* List of space separated list of MAC addresses to put into the mac filter */
} vif;
...
vif VIF =
{
	...
	.macfilter          = {NULL, "macfilter", {0}, NULL,  NULL},
...
void parse_uci(char * arg)
{
    ...
    uci_foreach_element(&uci_wireless->sections, e)
    {
        ...
        if(0 == strcmp(s->type, "wifi-iface"))
        {
			...
			PARSE_UCI_LIST(wifi_cfg[cur_dev].vifs[cur_vif].maclist);
#if 0
...
void hooker(FILE * fp, param * p, const char * devname)
{
	...
	else if(0 == strmatch(p->dat_key, "AccessPolicy?"))
	{
		...
		if (!strcmp(wifi_cfg[N].vifs[i].macfilter.value, "allow"))
	        FPRINT(fp, p, "1");
		else if (!strcmp(wifi_cfg[N].vifs[i].macfilter.value, "deny"))
	        FPRINT(fp, p, "2");
		else
	        FPRINT(fp, p, "0");
	}
